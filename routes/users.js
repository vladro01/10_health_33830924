...
// Helper: record auth events in login_audit
function recordAudit(req, userId, usernameAttempt, eventType) {
  const auditSql = `
...
// GET /users/login: show EJS login page
...
// POST /users/login: check bcrypt password
router.post('/login', (req, res, next) => {
  ...
  db.query(sql, [username], (err, results) => {
    ...
    bcrypt.compare(password, user.password_hash, (bcryptErr, isMatch) => {
      ...
      recordAudit(req, user.id, username, 'login_success');

      // After login, send them to the quests list
      res.redirect('/quests');
    });
  });
});
...
router.get('/logout', redirectLogin, (req, res, next) => {
  ...
  // Record logout event
  recordAudit(req, userId, username, 'logout');
  ...
  db.query(checkSql, [username], (err, rows) => {
    ...
    bcrypt.hash(password, 10, (hashErr, hash) => {
      ...
      db.query(insertSql, [username, hash], (insErr, result) => {
        ...
        recordAudit(req, result.insertId, username, 'register_success');
